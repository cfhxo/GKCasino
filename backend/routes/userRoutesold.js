...
// PayPal Wallet Topup
router.post("/wallet/topup", authMiddleware.isAuthenticated, async (req, res) => {
  try {
...
router.post('/claimBonus', authMiddleware.isAuthenticated, async (req, res) => {
  try {
    ...
    if (currentTime >= nextBonusTime) {
      ...
      user.nextBonus = new Date(currentTime.getTime() + 5 * 60000);      // Set next bonus time to 5 min later
      // Set bonus amount to 200 * 10% of the user current level
      user.bonusAmount = Math.floor(200 * (1 + 0.15 * user.level));

      // Save updated user
      await user.save();

      res.json({ message: `Claimed Centrelink Payment of G₽${currentBonus}!`, value: currentBonus, nextBonus: user.nextBonus });
...
router.get("/:id", async (req, res) => {
  ...
  try {
      console.log(`[FriendsService] Fetching data for user ID: ${userId}`);
      // Populate the `senderId` field in the `friendRequests` array
      const user = await User.findById(userId).populate("friendRequests.senderId", "id name email");

      if (!user) {
          console.log(`[FriendsService] User with ID ${userId} not found.`);
          return res.status(404).json({ message: "User not found" });
      }

      console.log(`[FriendsService] User data fetched successfully:`, user);
      res.json(user);
  } catch (error) {
      console.error(`[FriendsService] Error fetching user data for ID ${userId}:`, error);
      res.status(500).json({ message: "Internal server error" });
  }
});
