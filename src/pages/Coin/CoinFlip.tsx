...
const CoinFlip = () => {
  ...
  const { isLogged, userData, setUserData, toogleUserFlow } = useContext(UserContext);
  ...
  useEffect(() => {
    ...
    const userDataUpdatedListener = (updatedData: any) => {
    ...
    socket.on("userDataUpdated", userDataUpdatedListener);

    return () => {
      // Clean up listeners when the component is unmounted
      socket.off("coinFlip:start", startListener);
      socket.off("coinFlip:result", resultListener);
      socket.off("userDataUpdated", userDataUpdatedListener);
    };
  }, [choice, bet, userGambled, setUserData]);
        ...
        <div className="lg:w-[340px] flex flex-col items-center gap-4 border-r border-gray-700 py-4 px-6">
            ...
            <div className="flex items-center justify-between gap-2 w-full flex-col lg:flex-row">
              {[
                ...
                <button
                  ...
                  className={`p-2 border rounded w-1/2 bg-${e.color}-500 ${
                    choice === e.id && "bg-opacity-30"
                  }`}
                >
                  {e.name}
                </button>
          ...
          <button
            ...
            disabled={
              ...
              bet === 0 ||
              userGambled ||
              (userData !== null && userData.walletBalance < bet) ||
              spinning ||
              bet > 1000000
            }
          >
          ...
          <div className="flex w-screen lg:w-[800px] p-4 flex-col">
            ...
            <div className="flex items-center gap-2 justify-end w-full  overflow-hidden h-[24px]">
              {history.map((e, i) => (
                <motion.div
                  key={i}
                  className={`min-w-[24px] min-h-[24px] rounded-full ${
                    e.result === 0 ? "bg-red-500" : "bg-green-500"
                  }`}
                  initial={
                    i === history.length - 1 ? { opacity: 0, x: 30 } : {}
                  } // If this is the newest result, initialize animation state
                  animate={
                    i === history.length - 1 ? { opacity: 1, x: 0 } : {}
                  } // If this is the newest result, set final animation state
